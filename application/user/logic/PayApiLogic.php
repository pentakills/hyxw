...
class PayApiLogic extends Model
{
        ...
        if (isset($PayInfo['is_open_wechat']) && 0 == $PayInfo['is_open_wechat']) {
            if (!isMobile()) {
                ...
                if (!empty($Post['openid'])) {
                    // 小程序支付
                    $Paydata = model('PayApi')->getWechatPay($Post['openid'], $out_trade_no, $total_fee, '小程序支付', '小程序支付', 1);
    ...
    // 使用支付宝支付，读取数据调起支付功能
    public function UseAliPayPay($Post = [], $Order = [], $PayInfo = [])
    {
        $alipay_url = null;
    ...
    public function OrderProcessing($Post = [], $Order = [], $PayDetails = [], $Config = [])
    {
        if (1 == $Post['transaction_type']) {
            ...
            if (empty($Order['order_status'])) {
                ...
                // 订单更新
                $ResultID = $this->shop_order_db->where($OrderWhere)->update($OrderData);

                // 订单更新后续操作
                if (!empty($ResultID)) {
                    // 添加订单操作记录
                    AddOrderAction($Order['order_id'], $this->users_id, 0, 1, 0, 1, '支付成功', '使用'.$Config['pay_name'].'完成支付');

                    // 虚拟自动发货
                    model('Pay')->afterVirtualProductPay($OrderWhere);

                    // 支付方式
                    $pay_name = 'wechat' == $Config['pay_mark'] || 'alipay' == $Config['pay_mark'] ? $Config['pay_mark'] : 'other';

                    // 邮箱发送
                    $Result['email'] = GetEamilSendData(tpCache('smtp'), $this->users, $Order, 1, $pay_name);

                    // 手机发送
                    $Result['mobile'] = GetMobileSendData(tpCache('sms'), $this->users, $Order, 1, $pay_name);
